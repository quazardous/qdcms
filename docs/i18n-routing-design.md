...
## Mono-app cohabitation: `reservedPaths`

When qdadm and qdcms run as a single Vue app (e.g. qdadm under `/admin/` + qdcms at `/`), the catch-all generated by `buildRoutes` would otherwise swallow `/admin/foo` because the qdadm routes don't sit on the same router. Two options:

1. **Sub-domain** (`app.example.com` for qdadm + `example.com` for qdcms) — recommended for production. No router conflict, no special handling. Cross-origin locale sync via cookie or `localStorage` (same-site shared if both on `example.com`'s parent zone).

2. **Path prefix** (`example.com/admin` + `example.com`) — pass `reservedPaths` to `buildRoutes` so the catch-all skips them:
   ```ts
   buildRoutes(slugTable, {
     locales: ['en', 'fr'],
     defaultLocale: 'en',
   ...
   ```
   Generated catch-all becomes `/:pathMatch((?!admin(?:/|$)).*)*` — anything starting with `/admin/` or exactly `/admin` falls through to whatever route the host registers separately. The `/` → default-locale redirect only fires on exact `/`, so reserved paths are not affected by it either.

`detectLocale` is already safe because it only matches the URL prefix against `available` locales — `/admin/foo` returns `null` (not a locale), so cookie/Accept-Language take over.
